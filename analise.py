...
with open(f'промежуточные результаты/ВСЕ ГОДА.txt', 'w', encoding='utf-8') as file:
    # for article in all_articles:
    file.write(' '.join(all_articles) + '\n')
print('Файл записан')

# 1. TF-IDF анализ
tfidf_vectorizer = TfidfVectorizer(max_df=0.1, min_df=2, max_features=5000, stop_words='english')
tfidf_matrix = tfidf_vectorizer.fit_transform(all_articles)
# print(f'TF-IDF матрица: {tfidf_matrix}')

# 2. Оптимизация числа тем с использованием перплексии
def find_optimal_topics_perplexity(tfidf_matrix, min_topics=4, max_topics=20):
    perplexity_scores = []
...
# Подбор числа тем
# perplexity_scores = find_optimal_topics_perplexity(tfidf_matrix, min_topics=4, max_topics=15)
...
lda_model = LatentDirichletAllocation(
                                    ...
                                    random_state=42,
                                    max_iter=2000,
                                    doc_topic_prior=0.1,  # Регулирует распределение тем
                                    topic_word_prior=0.01  # Регулирует распределение слов в темах
                                    )
...
silhouette_scores = find_optimal_clusters(tfidf_matrix, min_clusters=10, max_clusters=20)
